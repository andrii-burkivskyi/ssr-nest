...
import {
  PluralType, CommandType, ParsedPluralValue, ParsedPluralRule,
...
const replaceParams = (str: string, values: CommonMap) => {
  ...
  return Object.entries(values).reduce((resultStr, [key, value]) => String(resultStr).replace(paramsInStringMatcher(key), String(value)), str);
};
...
const replacePlural = (str: string, values: CommonMap = {}) => {
  ...
  return str.replace(pluralMatching, (match, type, key, rules) => {
    ...
    if (key && rules && pluralValue !== undefined) {
      const parsedPluralValue = getPluralValue(pluralValue);
...
const getPluralType = (str: string) => String(str).toLocaleLowerCase() === '[c]' && PluralType.CARDINAL ||
...
const getPluralRule = (str: string) => {
  ...
  const [__, command, ruleValue] = str.trim().match(ruleMatching) || [];
  if (command) {
    const [__, strictMarker, restCommand] = command.match(ruleIsStrictMatching) || [];
    return {
...
const getPluralValue = (value: string | number) => {
  const [__, integer, fraction] = String(value).match(/^(\d+)[\,\.]?(\d*)$/i) || [];
  const isNumber = integer !== undefined;
  return {
    isNumber, value: String(value), integer, fraction,
  };
};

const getCardinalValue = (parsedRules: Array<Nullable<ParsedPluralRule>>, parsedValue: ParsedPluralValue, language: keyof Translation['languages']) => {
