  ...
  constructor(props?: InitProps<T>) {
    ...
    push = () => {
      ...
      let filter = Object.entries(this.getPaginationFilter()).reduce((acc, [filterKey, filterValue]) => {
        ...
        if (isNested) {
          let nestedFilterValue = flatten({ [queryKey]: filterValue }, { delimiter: '.', maxDepth: 2, safe: true }) as Record<string, any>;
          nestedFilterValue = Object.entries(nestedFilterValue).reduce((nestedAcc, [key, value]) => {
            if (isEmpty(value as Record<string, any>)) { return nestedAcc; }
        ...
        if (isEmpty(filterValue as Record<string, any>)) { return acc; }
    ...
    private updateFilterFromLocation = (browserQueryProps?: qs.ParsedQuery<string>) => {
      ...
      this.keys.forEach((key) => {
        const isNested = QueryExtractor(this).isNested(key);
        if (isNested) {
          const queryField: QueryBase<Record<string, any>> = this[key];
          queryField.updateFilterFromLocation(browserQuery);
        } else {
          const queryProperty: QueryFieldBase<Record<string, any>> = this[key];
    ...
    private getPaginationFilter = () => this.keys.reduce((acc, key) => {
      const isNested = QueryExtractor(this).isNested(key);
      if (isNested) {
        const queryField: QueryBase<Record<string, any>> = this[key];
    ...
    getPagination = () => this.beforeGetPagination(clearUndefinedValues({
