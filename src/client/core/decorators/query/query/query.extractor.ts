...
export const QueryExtractor = (target: Record<string, any>) => ({
  keys: (Reflect.getMetadata(QFDKeys.QUERY_FIELD_KEYS, target) ?? DEFAULT_ARRAY) as string[],
  isNested: (propertyKey: string) => Reflect.getMetadata(QFDKeys.QUERY_FIELD_IS_NESTED, target, propertyKey) as boolean,
  Constructor: (propertyKey: string) => Reflect.getMetadata(
    QFDKeys.QUERY_FIELD_CONSTRUCTOR,
    target,
    propertyKey,
  ) as Constructable<QueryFieldBase<Record<string, any>>> | Constructable<QueryBase<Record<string, any>>>,
});
